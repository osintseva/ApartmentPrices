...
import joblib  # Import joblib for loading encoders
...
try:
    ...
    st.error("Label encoders file 'label_encoders.pkl' not found. Ensure it exists in the working directory.")
    st.stop()

# Load the dataset to access original data for distance calculations
df = pd.read_csv('df_full.csv')

# Define District IDs and Names
ids = [
...
# Function to calculate airline distance (consistent with training)
...
# Define Metro Stations with their coordinates
metro_stations = {
...
with col1:
    area_total = st.number_input('Total Area (sq m)', min_value=0.0, format="%.2f")
    kitchen_area = st.number_input('Kitchen Area (sq m)', min_value=0.0, format="%.2f")
...
with col2:
    construction_type = st.selectbox(
        'Construction Type', 
        ['монолітно-каркасний', 'цегляний будинок', 'утеплена панель', 'панельні']
    )
...
def create_map():
    ...
    folium.ClickForMarker(popup="Selected Location").add_to(kyiv_map)
    return kyiv_map
...
if st.session_state['location']:
    ...
    st.write('If you click on the marker twice, it will be deleted.')

# Button to clear the marker
if st.button('Clear Marker'):
    st.session_state['location'] = None

# Proceed only if a location is selected
if st.session_state['location']:
    lat, lon = st.session_state['location']
    
    # Calculate distance to all entries in the dataset to find the nearest ones
    df['distance'] = df.apply(lambda row: calculate_airline_distance(lat, lon, row['Latitude'], row['Longitude']), axis=1)
    ...
    district_name = district_dict.get(most_common_district_id, "Unknown District")
    ...
    # Uncomment below line if you want to display the distance
    # st.write(f"Distance to City Center: {distance_to_center:.4f} units")
    
    # Calculate distance to all metro stations and find the minimum distance
    metro_distances = {station: calculate_airline_distance(lat, lon, coords[0], coords[1]) for station, coords in metro_stations.items()}
    min_distance_to_metro = min(metro_distances.values())
    min_station_name = min(metro_distances, key=metro_distances.get)
    
    # Uncomment below lines if you want to display the nearest metro station
    # st.write(f"Nearest Metro Station: {min_station_name} at {min_distance_to_metro:.4f} units")
    
    def prepare_input_data():
        # Encode categorical features using the loaded LabelEncoders
        try:
            district_id_encoded = label_encoders['District ID'].transform([most_common_district_id])[0]
        except ValueError:
            st.error("Selected District ID was not seen during training.")
            return None
        
        try:
            construction_type_encoded = label_encoders['Construction Type'].transform([construction_type])[0]
        except ValueError:
            st.error("Selected Construction Type was not seen during training.")
            return None
        
        # Prepare the input DataFrame
        input_df = pd.DataFrame({
            'District ID': [district_id_encoded],
            ...
            'distance_to_nearest_metro_stations_m': [min_distance_to_metro * 1000]  # Convert to meters if needed
        })
    ...
    if st.button('Predict Price'):
        ...
        if input_data is not None:
            # Ensure the input data has the same feature order as training
            feature_order = [
                'District ID', 'Construction Type', 'furnished', 'appliances', 'security_features',
                'Rooms', 'Area_total', 'Kitchen_area', 'Storeys', 'Floor',
                'Building_Age', 'distance_to_center', 'renovation_quality', 'distance_to_nearest_metro_stations_m'
            ]
            ...
            predicted_price = np.exp(prediction_log_price)[0]
            
            st.success(f"Predicted Apartment Price: {predicted_price:,.2f} UAH")
else:
    st.warning("Please select a location on the map to proceed with prediction.")
